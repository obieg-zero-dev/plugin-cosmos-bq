...
git clone https://github.com/obieg-zero-dev/plugin-cosmos-bq    # to repo
...
cd ../plugin-cosmos-bq && git add -A && git commit -m "..."
```
...
```bash
bash core-host/scripts/promote-to-dev.sh  plugin-cosmos-bq             # LOCAL -> DEV
bash core-host/scripts/promote-to-prod.sh plugin-cosmos-bq v1.2.3      # DEV   -> PROD + tag
```
